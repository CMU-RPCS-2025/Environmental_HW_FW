...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMy_Custom_ServiceHdle;                    /**< MY_CUSTOM_SERVICE handle */
  uint16_t  CustomMy_Char_ChangethisHdle;                  /**< MY_CHAR_changethis handle */
  uint16_t  CustomTest2Hdle;                    /**< Service_Test_2 handle */
  uint16_t  CustomTest_2_CharHdle;                  /**< TEST_2_CHAR handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeMy_Char_Changethis = 1;
uint16_t SizeTest_2_Char = 1;
...
#define COPY_MY_CUSTOM_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_MY_CHAR_CHANGETHIS_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_SERVICE_TEST_2_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_TEST_2_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *
   * Max_Attribute_Records = 1 + 2*1 + 1*no_of_char_with_notify_or_indicate_property + 1*no_of_char_with_broadcast_property
   * service_max_attribute_record = 1 for Service_Test_2 +
   *                                2 for TEST_2_CHAR +
   *                              = 3
   *
   * This value doesn't take into account number of descriptors manually added
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MY_CHAR_CHANGETHIS:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_Custom_ServiceHdle,
                                       CustomContext.CustomMy_Char_ChangethisHdle,
                                       0, /* charValOffset */
                                       SizeMy_Char_Changethis, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MY_CHAR_CHANGETHIS command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MY_CHAR_CHANGETHIS command\n\r");
      }
    ...
    case CUSTOM_STM_TEST_2_CHAR:
      ret = aci_gatt_update_char_value(CustomContext.CustomTest2Hdle,
                                       CustomContext.CustomTest_2_CharHdle,
                                       0, /* charValOffset */
                                       SizeTest_2_Char, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value TEST_2_CHAR command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value TEST_2_CHAR command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MY_CHAR_CHANGETHIS:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMy_Custom_ServiceHdle, CustomContext.CustomMy_Char_ChangethisHdle, SizeMy_Char_Changethis, pPayload);
    ...
    case CUSTOM_STM_TEST_2_CHAR:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomTest2Hdle, CustomContext.CustomTest_2_CharHdle, SizeTest_2_Char, pPayload);
